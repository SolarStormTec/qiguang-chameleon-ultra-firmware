...
static bool sleep_preconditions_ok(void) {
    if (nrfx_power_usbstatus_get() != NRFX_POWER_USB_STATE_DISCONNECTED) {
        return false;
    }
    return (g_is_tag_emulating == false) && (m_is_field_on == false);
}
...
bool sleep_timer_can_sleep_after_disconnect(void) {
    return sleep_preconditions_ok();
}

void sleep_timer_start(uint32_t time_ms) {
    // Close the previous sleep timer first
    sleep_timer_stop();
    if (sleep_preconditions_ok() && (g_is_ble_connected == false)) {
