...
APP_TIMER_DEF(m_ble_idle_timer);
static volatile bool m_idle_activity;
static volatile uint16_t m_idle_ticks;
static volatile bool m_idle_sleep_armed;
static bool          m_idle_disconnect_ours;
...
static void on_conn_params_evt(ble_conn_params_evt_t *p_evt) {
    if (p_evt->evt_type == BLE_CONN_PARAMS_EVT_FAILED) {
        NRF_LOG_WARNING("conn params negotiation gave up; keeping the link as-is");
    }
}
...
static void ble_evt_handler(ble_evt_t const *p_ble_evt, void *p_context) {
    ...
    switch (p_ble_evt->header.evt_id) {
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            if (m_idle_disconnect_ours) {
                m_idle_disconnect_ours = false;
                NRF_LOG_INFO("Idle sleep: stay silent, no re-advertising");
            } else {
                ret_code_t adv_err = ble_advertising_start(&m_advertising, BLE_ADV_MODE_FAST);
                if (adv_err != NRF_SUCCESS) {
                    NRF_LOG_WARNING("Re-advertising after disconnect failed: %d", adv_err);
                }
            }
            sky_event_on_disconnect();
            sky_cycle_note_link_down();
...
#define BATT_SAMPLE_COUNT   5u
#define BATT_REPORT_STEP    2u
#define BATT_REPORT_JUMP    10u
#define BATT_ZERO_CONFIRM   2u
#define BATT_TREND_COUNT    3u

static bool    m_batt_last_valid;
static uint8_t m_batt_last_measured;
static uint8_t m_batt_zero_streak;
static bool    m_batt_sampled;
static uint16_t m_batt_mv_trend[BATT_TREND_COUNT];
static uint8_t  m_batt_mv_head;
static uint8_t  m_batt_mv_len;
static bool    m_power_flags_valid;
static uint8_t m_power_flags_last;
...
static uint16_t battery_trend_mv(uint16_t mv_now) {
    m_batt_mv_trend[m_batt_mv_head] = mv_now;
    m_batt_mv_head = (uint8_t)((m_batt_mv_head + 1u) % BATT_TREND_COUNT);
    if (m_batt_mv_len < BATT_TREND_COUNT) {
        m_batt_mv_len++;
    }
    uint16_t window[BATT_TREND_COUNT];
    for (uint8_t i = 0; i < m_batt_mv_len; i++) {
        window[i] = m_batt_mv_trend[i];
    }
    return median_u16(window, m_batt_mv_len);
}

static uint16_t battery_read_mv_median(void) {
    uint16_t v[BATT_SAMPLE_COUNT];
    for (uint8_t i = 0; i < BATT_SAMPLE_COUNT; i++) {
        nrf_saadc_value_t adc_result;
        ret_code_t err_code = nrfx_saadc_sample_convert(ADC_CHANNEL, &adc_result);
        APP_ERROR_CHECK(err_code);
        v[i] = (uint16_t)(ADC_RESULT_IN_MILLI_VOLTS(adc_result) + 100);
    }
    return median_u16(v, BATT_SAMPLE_COUNT);
}

uint8_t ble_power_flags(void) {
    uint8_t flags = 0;
    if (nrfx_power_usbstatus_get() != NRFX_POWER_USB_STATE_DISCONNECTED) {
        flags |= SKY_POWER_FLAG_VBUS;
    }
    if (m_batt_sampled == false) {
        flags |= SKY_POWER_FLAG_UNSAMPLED;
    } else if (percentage_batt_lvl >= 100u) {
        flags |= SKY_POWER_FLAG_FULL;
    }
    return flags;
}
...
static void battery_level_meas_timeout_handler(void *p_context) {
    ...
    uint16_t mv = battery_trend_mv(battery_read_mv_median());
    uint8_t measured = (uint8_t)BATVOL2PERCENT(mv);
    batt_lvl_in_milli_volts = mv;
    m_batt_sampled = true;

    uint8_t prev_pct = percentage_batt_lvl;
    uint16_t diff = (measured > percentage_batt_lvl) ? (uint16_t)(measured - percentage_batt_lvl)
                                                     : (uint16_t)(percentage_batt_lvl - measured);
    uint16_t drift = (measured > m_batt_last_measured) ? (uint16_t)(measured - m_batt_last_measured)
                                                       : (uint16_t)(m_batt_last_measured - measured);
    bool confirmed = m_batt_last_valid && (drift <= 1u);
    if (m_batt_last_valid == false) {
        percentage_batt_lvl = measured;
    } else if (diff >= BATT_REPORT_JUMP) {
        percentage_batt_lvl = measured;
    } else if ((diff >= BATT_REPORT_STEP) && confirmed) {
        percentage_batt_lvl = measured;
    }
    m_batt_last_measured = measured;
    m_batt_last_valid = true;

    if (percentage_batt_lvl != prev_pct) {
        sky_event_note_battery(batt_lvl_in_milli_volts, (uint8_t)percentage_batt_lvl);
    }

    uint8_t power_flags = ble_power_flags();
    if (m_power_flags_valid == false || power_flags != m_power_flags_last) {
        m_power_flags_valid = true;
        m_power_flags_last = power_flags;
        sky_event_note_power(batt_lvl_in_milli_volts, (uint8_t)percentage_batt_lvl, power_flags);
    }

    // if battery service is notification enable, we can send msg to device.
    ret_code_t err_code = ble_bas_battery_level_update(&m_bas, percentage_batt_lvl, BLE_CONN_HANDLE_ALL);
    if ((err_code != NRF_SUCCESS) &&
    ...
    if (measured == 0) {
        if (m_batt_zero_streak < 255) {
            m_batt_zero_streak++;
        }
    } else {
        m_batt_zero_streak = 0;
    }
    if (m_batt_zero_streak >= BATT_ZERO_CONFIRM) {
...
#define BLE_IDLE_TICK_S 15u

static uint16_t ble_idle_target_ticks(void) {
    const uint16_t seconds = settings_get_ble_idle_seconds();
    if (seconds == 0u) return 0u;
    return (uint16_t)((seconds + BLE_IDLE_TICK_S - 1u) / BLE_IDLE_TICK_S);
}

STATIC_ASSERT((SETTINGS_BLE_IDLE_MAX_S / BLE_IDLE_TICK_S) < 0xFFFFu);

static bool ble_idle_applicable(void) {
    if (nrfx_power_usbstatus_get() != NRFX_POWER_USB_STATE_DISCONNECTED) {
        return false;
    }
    return g_is_ble_connected;
...
static void ble_idle_tick_handler(void *p_context) {
    UNUSED_PARAMETER(p_context);

    if (ble_idle_applicable() == false) {
        m_idle_activity    = false;
        m_idle_ticks       = 0;
        m_idle_sleep_armed = false;
        return;
    }

    const uint16_t target = ble_idle_target_ticks();
    if (target == 0u) {
        m_idle_activity    = false;
        m_idle_ticks       = 0;
        m_idle_sleep_armed = false;
    ...
    if (m_idle_ticks < target) {
        m_idle_ticks++;
    }
    if (m_idle_ticks >= target) {
        m_idle_sleep_armed = true;
    }
}

void ble_idle_init(void) {
    ret_code_t err_code;

    err_code = app_timer_create(&m_ble_idle_timer, APP_TIMER_MODE_REPEATED, ble_idle_tick_handler);
    APP_ERROR_CHECK(err_code);
    err_code = app_timer_start(m_ble_idle_timer, APP_TIMER_TICKS(BLE_IDLE_TICK_S * 1000u), NULL);
    APP_ERROR_CHECK(err_code);
}

void ble_idle_apply_config(void) {
    m_idle_ticks       = 0;
    m_idle_activity    = false;
    m_idle_sleep_armed = false;
}

uint8_t ble_idle_flags(void) {
    uint8_t flags = 0;
    if (ble_idle_applicable()) flags |= 0x01u;
    if (m_idle_sleep_armed)    flags |= 0x02u;
    return flags;
}

void ble_idle_note_activity(void) {
    m_idle_activity    = true;
    m_idle_sleep_armed = false;
}

void ble_idle_process(void) {
    if (m_idle_sleep_armed == false) {
        return;
    }
    ...
    if (sleep_timer_can_sleep_after_disconnect() == false) {
        m_idle_sleep_armed = false;
        return;
    }
    ...
    m_idle_disconnect_ours = true;

    ret_code_t err_code = sd_ble_gap_disconnect(m_conn_handle, BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
    if (err_code != NRF_SUCCESS) {
        m_idle_disconnect_ours = false;
        NRF_LOG_WARNING("Idle disconnect failed: %d", err_code);
    } else {
        NRF_LOG_INFO("BLE idle %u s: disconnecting to sleep", (unsigned)settings_get_ble_idle_seconds());
    }
}
