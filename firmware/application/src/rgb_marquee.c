...
typedef enum { SW_IDLE = 0, SW_FADE_OUT, SW_FADE_IN } slot_switch_phase_t;
static slot_switch_phase_t m_sw_phase = SW_IDLE;
static uint32_t m_sw_t0 = 0;
static int16_t m_sw_level = -1;
...
#define SLOT_SWITCH_FADE_MS     143
#define SLOT_SWITCH_LEVEL_MAX   99
#define SLOT_SWITCH_NO_LED      0xFF

static uint8_t m_sw_led_down = SLOT_SWITCH_NO_LED;
static uint8_t m_sw_led_up = SLOT_SWITCH_NO_LED;
static uint8_t m_sw_color_down = 0;
static uint8_t m_sw_color_up = 0;

static void slot_switch_draw(uint8_t led_clear, uint8_t color, int16_t level) {
    uint32_t *led_pins = hw_get_led_array();

    pwm_sequ_val.channel_0 = get_pwmduty((uint8_t)level);

    nrfx_pwm_uninit(&pwm0_ins);

    if (led_clear <= 7) {
        nrf_gpio_pin_clear(led_pins[led_clear]);
    }
...
static void slot_switch_target(uint8_t led) {
    uint32_t *led_pins = hw_get_led_array();
    pwm_config.output_pins[0] = led_pins[led];
    pwm_config.output_pins[1] = NRF_DRV_PWM_PIN_NOT_USED;
    pwm_config.output_pins[2] = NRF_DRV_PWM_PIN_NOT_USED;
    pwm_config.output_pins[3] = NRF_DRV_PWM_PIN_NOT_USED;
}

static void slot_switch_finish(void) {
    uint32_t *led_pins = hw_get_led_array();
    nrfx_pwm_uninit(&pwm0_ins);
...
static void slot_switch_begin_fade_in(void) {
    uint32_t *led_pins = hw_get_led_array();
    for (uint8_t i = 0; i < RGB_LIST_NUM; i++) {
        nrf_gpio_pin_clear(led_pins[i]);
    }
    ...
    slot_switch_target(m_sw_led_up);
    m_sw_phase = SW_FADE_IN;
    m_sw_t0 = app_timer_cnt_get();
    m_sw_level = 0;
    slot_switch_draw(m_sw_led_down, m_sw_color_up, 0);
}

bool rgb_marquee_slot_switch_busy(void) {
    return m_sw_phase != SW_IDLE;
}

void rgb_marquee_tick(void) {
    if (m_sw_phase == SW_IDLE) return;

    const uint32_t total = APP_TIMER_TICKS(SLOT_SWITCH_FADE_MS);
    uint32_t elapsed = app_timer_cnt_diff_compute(app_timer_cnt_get(), m_sw_t0);

    if (elapsed >= total) {
        if (m_sw_phase == SW_FADE_OUT) slot_switch_begin_fade_in();
        else slot_switch_finish();
        return;
    }

    int16_t ramp = (int16_t)((elapsed * SLOT_SWITCH_LEVEL_MAX) / total);
    int16_t level = (m_sw_phase == SW_FADE_OUT) ? (SLOT_SWITCH_LEVEL_MAX - ramp) : ramp;

    if (level == m_sw_level) return;
    m_sw_level = level;

    if (m_sw_phase == SW_FADE_OUT) slot_switch_draw(m_sw_led_up, m_sw_color_down, level);
    else slot_switch_draw(m_sw_led_down, m_sw_color_up, level);
}

void rgb_marquee_slot_switch(uint8_t led_down, uint8_t color_led_down, uint8_t led_up, uint8_t color_led_up) {
    m_sw_led_down = (led_down < RGB_LIST_NUM) ? led_down : SLOT_SWITCH_NO_LED;
    m_sw_led_up = (led_up < RGB_LIST_NUM) ? led_up : SLOT_SWITCH_NO_LED;
    m_sw_color_down = color_led_down;
    m_sw_color_up = color_led_up;

    if (m_sw_led_down > 7) {
        slot_switch_begin_fade_in();
        return;
    }

    slot_switch_target(m_sw_led_down);
    m_sw_phase = SW_FADE_OUT;
    m_sw_t0 = app_timer_cnt_get();
    m_sw_level = SLOT_SWITCH_LEVEL_MAX;
    slot_switch_draw(m_sw_led_up, m_sw_color_down, SLOT_SWITCH_LEVEL_MAX);
}
