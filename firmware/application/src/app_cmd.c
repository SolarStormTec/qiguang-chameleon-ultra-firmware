...
static void change_slot_auto(uint8_t slot_new, uint8_t reason) {
    uint8_t slot_now = tag_emulation_get_slot();
    device_mode_t mode = get_device_mode();
    if (reason != SKY_SLOT_CHANGE_BY_AUTO) {
        sky_cycle_note_slot_taken_over();
    }
    if (nfc_tag_14a_field_present()) {
        nfc_tag_14a_note_slotcut_in_field();
    }
    tag_emulation_change_slot(slot_new, mode != DEVICE_MODE_READER);
    apply_slot_change(slot_now, slot_new);
    sky_event_note_slot_changed(slot_new, reason);
}
...
static data_frame_tx_t *cmd_processor_set_active_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    change_slot_auto(data[0], SKY_SLOT_CHANGE_BY_APP);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}
...
static data_frame_tx_t *cmd_processor_set_slot_data_default(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    status = tag_emulation_factory_data(payload->num_slot, tag_type) ? STATUS_SUCCESS : STATUS_FLASH_WRITE_FAIL;
    return data_frame_make(cmd, status, 0, NULL);
}
...
static data_frame_tx_t *cmd_processor_set_slot_enable(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    if ((!payload->enabled) &&
        ...
        if (slot_prev == slot_now) {
            ...
            change_slot_auto(slot_prev, SKY_SLOT_CHANGE_BY_FIRMWARE);
        }
    }
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}
...
#define LEGACY_NICKS_SLOT_NUM 8

static data_frame_tx_t *cmd_processor_get_all_slot_nicks(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint8_t response_buffer[LEGACY_NICKS_SLOT_NUM * 2 * 37];
    uint16_t response_length = 0;

    for (uint8_t slot = 0; slot < MIN(LEGACY_NICKS_SLOT_NUM, TAG_MAX_SLOT_NUM); slot++) {
...
#define SLOT_TABLE_BUF_SIZE   2048
#define SLOT_TABLE_HEADER_LEN 4
#define SLOT_TABLE_NICK_BUF   36

static uint8_t m_slot_table_buf[SLOT_TABLE_BUF_SIZE];

static uint8_t slot_table_read_nick(uint8_t slot, tag_sense_type_t sense_type, uint8_t *out) {
    uint8_t buf[SLOT_TABLE_NICK_BUF];
    fds_slot_record_map_t map_info;
    get_fds_map_by_slot_sense_type_for_nick(slot, sense_type, &map_info);
    uint16_t buf_len = sizeof(buf);
    if (!fds_read_sync(map_info.id, map_info.key, &buf_len, buf) || buf_len == 0) {
        return 0;
    }
    uint8_t n = buf[0];
    if (n > buf_len - 1) n = (uint8_t)(buf_len - 1);
    if (n > SLOT_TABLE_NICK_BUF - 1) n = SLOT_TABLE_NICK_BUF - 1;
    memcpy(out, &buf[1], n);
    return n;
}

static data_frame_tx_t *cmd_processor_get_slot_table(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (length != 2) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    uint8_t start = data[0];
    uint8_t max_count = data[1];
    if (start >= TAG_MAX_SLOT_NUM) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    if (max_count == 0) max_count = TAG_MAX_SLOT_NUM;

    uint8_t *p = m_slot_table_buf + SLOT_TABLE_HEADER_LEN;
    const uint8_t *end = m_slot_table_buf + SLOT_TABLE_BUF_SIZE;
    uint8_t count = 0;

    for (uint8_t slot = start; slot < TAG_MAX_SLOT_NUM && count < max_count; slot++) {
        bsp_wdt_feed();

        uint8_t hf_nick[SLOT_TABLE_NICK_BUF], lf_nick[SLOT_TABLE_NICK_BUF];
        uint8_t hf_len = slot_table_read_nick(slot, TAG_SENSE_HF, hf_nick);
        uint8_t lf_len = slot_table_read_nick(slot, TAG_SENSE_LF, lf_nick);

        size_t need = 4 + 2 + 1 + (size_t)hf_len + 1 + (size_t)lf_len;
        if ((size_t)(end - p) < need) break;

        tag_slot_specific_type_t types;
        tag_emulation_get_specific_types_by_slot(slot, &types);
        *p++ = (uint8_t)(types.tag_hf >> 8);
        *p++ = (uint8_t)(types.tag_hf & 0xFF);
        *p++ = (uint8_t)(types.tag_lf >> 8);
        *p++ = (uint8_t)(types.tag_lf & 0xFF);
        *p++ = is_slot_enabled(slot, TAG_SENSE_HF) ? 1 : 0;
        *p++ = is_slot_enabled(slot, TAG_SENSE_LF) ? 1 : 0;
        *p++ = hf_len;
        memcpy(p, hf_nick, hf_len);
        p += hf_len;
        *p++ = lf_len;
        memcpy(p, lf_nick, lf_len);
        p += lf_len;
        count++;
    }

    m_slot_table_buf[0] = start;
    m_slot_table_buf[1] = count;
    m_slot_table_buf[2] = (uint8_t)(TAG_MAX_SLOT_NUM >> 8);
    m_slot_table_buf[3] = (uint8_t)(TAG_MAX_SLOT_NUM & 0xFF);
    return data_frame_make(cmd, STATUS_SUCCESS, (uint16_t)(p - m_slot_table_buf), m_slot_table_buf);
}
...
static data_frame_tx_t *cmd_processor_sky_activate_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_read_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_write_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_set_event_mask(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_get_fds_stat(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_get_power(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_get_nfc_stat(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_get_slot_health(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_get_nfct_diag(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_get_ble_idle(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
static data_frame_tx_t *cmd_processor_sky_set_ble_idle(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data);
...
static data_frame_tx_t *cmd_processor_get_attestation(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (!device_attest_has_key()) {
        return data_frame_make(cmd, STATUS_NOT_IMPLEMENTED, 0, NULL);
    }
    if (data == NULL || length < 8 || length > 64) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    uint8_t resp[ATTEST_CHIP_ID_LEN + ATTEST_SIG_LEN + ATTEST_CERT_LEN];
    device_attest_get_chip_id(resp);
    if (device_attest_sign(data, length, resp + ATTEST_CHIP_ID_LEN) != 0) {
        return data_frame_make(cmd, STATUS_CMD_ERR, 0, NULL);
    }
    uint16_t rlen = ATTEST_CHIP_ID_LEN + ATTEST_SIG_LEN;
    if (device_attest_get_cert(resp + rlen)) {
        rlen += ATTEST_CERT_LEN;
    }
    return data_frame_make(cmd, STATUS_SUCCESS, rlen, resp);
}

static data_frame_tx_t *cmd_processor_attest_provision_init(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (!is_usb_working()) {
        return data_frame_make(cmd, STATUS_INVALID_CMD, 0, NULL);
    }
    uint8_t resp[ATTEST_CHIP_ID_LEN + ATTEST_PUB_LEN];
    device_attest_get_chip_id(resp);
    if (device_attest_ensure_key(resp + ATTEST_CHIP_ID_LEN) != 0) {
        return data_frame_make(cmd, STATUS_CMD_ERR, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_SUCCESS, sizeof(resp), resp);
}

static data_frame_tx_t *cmd_processor_attest_provision_set_cert(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (!is_usb_working()) {
        return data_frame_make(cmd, STATUS_INVALID_CMD, 0, NULL);
    }
    if (data == NULL || length != ATTEST_CERT_LEN) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    if (device_attest_set_cert(data) != 0) {
        return data_frame_make(cmd, STATUS_CMD_ERR, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_attest_provision_lock(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    if (!is_usb_working()) {
        return data_frame_make(cmd, STATUS_INVALID_CMD, 0, NULL);
    }
    if (device_attest_lock() != 0) {
        return data_frame_make(cmd, STATUS_CMD_ERR, 0, NULL);
    }
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}
...
static cmd_data_map_t m_data_cmd_map[] = {
    ...
    {    DATA_CMD_GET_SLOT_TABLE,               NULL,                        cmd_processor_get_slot_table,                NULL                   },
    {    DATA_CMD_GET_ATTESTATION,              NULL,                        cmd_processor_get_attestation,               NULL                   },
    {    DATA_CMD_ATTEST_PROVISION_INIT,        NULL,                        cmd_processor_attest_provision_init,         NULL                   },
    {    DATA_CMD_ATTEST_PROVISION_SET_CERT,    NULL,                        cmd_processor_attest_provision_set_cert,     NULL                   },
    {    DATA_CMD_ATTEST_PROVISION_LOCK,        NULL,                        cmd_processor_attest_provision_lock,         NULL                   },

    {    DATA_CMD_SKY_ACTIVATE_SLOT,            NULL,                        cmd_processor_sky_activate_slot,             NULL                   },
    {    DATA_CMD_SKY_WRITE_SLOT,               NULL,                        cmd_processor_sky_write_slot,                NULL                   },
    {    DATA_CMD_SKY_READ_SLOT,                NULL,                        cmd_processor_sky_read_slot,                 NULL                   },
    {    DATA_CMD_SKY_GET_FDS_STAT,             NULL,                        cmd_processor_sky_get_fds_stat,              NULL                   },
    {    DATA_CMD_SKY_GET_POWER,                NULL,                        cmd_processor_sky_get_power,                 NULL                   },
    {    DATA_CMD_SKY_GET_NFC_STAT,             NULL,                        cmd_processor_sky_get_nfc_stat,              NULL                   },
    {    DATA_CMD_SKY_GET_SLOT_HEALTH,          NULL,                        cmd_processor_sky_get_slot_health,           NULL                   },
    {    DATA_CMD_SKY_GET_NFCT_DIAG,            NULL,                        cmd_processor_sky_get_nfct_diag,             NULL                   },
    {    DATA_CMD_SKY_SET_EVENT_MASK,           NULL,                        cmd_processor_sky_set_event_mask,            NULL                   },
    {    DATA_CMD_SKY_GET_BLE_IDLE,             NULL,                        cmd_processor_sky_get_ble_idle,              NULL                   },
    {    DATA_CMD_SKY_SET_BLE_IDLE,             NULL,                        cmd_processor_sky_set_ble_idle,              NULL                   },
...
static data_frame_tx_t *cmd_processor_sky_activate_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    if (tag_emulation_get_slot() != slot) {
        change_slot_auto(slot, SKY_SLOT_CHANGE_BY_APP);
    }
    tag_emulation_slot_set_enable(slot, TAG_SENSE_HF, true);
...
#define SKY_SLOT_WIRE_VER   1
#define SKY_HAS_VERSION     0x01
#define SKY_HAS_SIGNATURE   0x02
#define SKY_HAS_COUNTER     0x04
#define SKY_HAS_NICK        0x08
#define SKY_SLOT_PAYLOAD_MAX 1400
static uint8_t m_sky_slot_buf[SKY_SLOT_PAYLOAD_MAX];

static uint8_t sky_read_slot_nick(uint8_t slot, uint8_t *out, uint8_t cap) {
    fds_slot_record_map_t map_info;
    get_fds_map_by_slot_sense_type_for_nick(slot, TAG_SENSE_HF, &map_info);
    uint8_t buffer[36];
    uint16_t length = sizeof(buffer);
    if (!fds_read_sync(map_info.id, map_info.key, &length, buffer)) return 0;
    uint8_t n = buffer[0];
    if (n == 0 || n > 32 || n > cap) return 0;
    memcpy(out, buffer + 1, n);
    return n;
}
...
static data_frame_tx_t *cmd_processor_sky_read_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    int nr_pages = nfc_tag_mf0_ntag_get_nr_pages_by_tag_type(types.tag_hf);
    if (nr_pages <= 0) {
        return data_frame_make(cmd, STATUS_INVALID_SLOT_TYPE, 0, NULL);
    }
    ...
    nfc_tag_14a_coll_res_reference_t *coll = get_coll_res_data(false);
    if (coll == NULL) {
        return data_frame_make(cmd, STATUS_INVALID_SLOT_TYPE, 0, NULL);
    }
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(types.tag_hf);
    if (buffer == NULL) {
        return data_frame_make(cmd, STATUS_INVALID_SLOT_TYPE, 0, NULL);
    }
    nfc_tag_mf0_ntag_information_t *info = (nfc_tag_mf0_ntag_information_t *)buffer->buffer;

    uint8_t *p = m_sky_slot_buf;
    *p++ = SKY_SLOT_WIRE_VER;
    *p++ = (types.tag_hf >> 8) & 0xFF;
    *p++ = types.tag_hf & 0xFF;

    uint8_t uid_len = (uint8_t)(*coll->size);
    *p++ = uid_len;
    memcpy(p, coll->uid, uid_len);
    p += uid_len;
    memcpy(p, coll->atqa, 2);
    p += 2;
    *p++ = coll->sak[0];
    uint8_t ats_len = coll->ats->length;
    *p++ = ats_len;
    if (ats_len > 0) {
        memcpy(p, coll->ats->data, ats_len);
        p += ats_len;
    }

    *p++ = (uint8_t)nr_pages;
    memcpy(p, &info->memory[0][0], (size_t)nr_pages * NFC_TAG_MF0_NTAG_DATA_SIZE);
    p += (size_t)nr_pages * NFC_TAG_MF0_NTAG_DATA_SIZE;

    uint8_t *has = p++;
    *has = 0;

    uint8_t *ver = nfc_tag_mf0_ntag_get_version_data();
    if (ver != NULL) {
        *has |= SKY_HAS_VERSION;
        memcpy(p, ver, NFC_TAG_MF0_NTAG_VER_SIZE);
        p += NFC_TAG_MF0_NTAG_VER_SIZE;
    }
    uint8_t *sig = nfc_tag_mf0_ntag_get_signature_data();
    if (sig != NULL) {
        *has |= SKY_HAS_SIGNATURE;
        memcpy(p, sig, NFC_TAG_MF0_NTAG_SIG_SIZE);
        p += NFC_TAG_MF0_NTAG_SIG_SIZE;
    }
    ...
    uint8_t nick_len = sky_read_slot_nick(slot, p + 1, 32);
    if (nick_len > 0) {
        *has |= SKY_HAS_NICK;
        *p++ = nick_len;
        p += nick_len;
    ...
    return data_frame_make(cmd, STATUS_SUCCESS, (uint16_t)(p - m_sky_slot_buf), m_sky_slot_buf);
}


static bool sky_write_rollback(uint8_t slot) {
    tag_emulation_delete_data(slot, TAG_SENSE_HF);
    return tag_emulation_save();
}

static data_frame_tx_t *sky_write_failed(uint16_t cmd, uint16_t status, uint8_t slot) {
    return data_frame_make(cmd, sky_write_rollback(slot) ? status : STATUS_FLASH_WRITE_FAIL, 0, NULL);
}

static data_frame_tx_t *cmd_processor_sky_write_slot(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint16_t o = 0;
    #define SKY_NEED(n) do { if ((uint32_t)o + (n) > length) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL); } while (0)

    SKY_NEED(1); uint8_t wire_ver = data[o++];
    if (wire_ver != SKY_SLOT_WIRE_VER) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    SKY_NEED(1); uint8_t slot = data[o++];
    SKY_NEED(2); uint16_t flags = ((uint16_t)data[o] << 8) | data[o + 1]; o += 2;
    SKY_NEED(2); tag_specific_type_t tag_type = (tag_specific_type_t)(((uint16_t)data[o] << 8) | data[o + 1]); o += 2;
    SKY_NEED(1); uint8_t uid_len = data[o++];
    SKY_NEED(uid_len); uint8_t *uid = &data[o]; o += uid_len;
    SKY_NEED(2); uint8_t *atqa = &data[o]; o += 2;
    SKY_NEED(1); uint8_t sak = data[o++];
    SKY_NEED(1); uint8_t ats_len = data[o++];
    SKY_NEED(ats_len); uint8_t *ats = &data[o]; o += ats_len;
    SKY_NEED(1); uint8_t page_count = data[o++];
    SKY_NEED((uint32_t)page_count * NFC_TAG_MF0_NTAG_DATA_SIZE);
    uint8_t *pages = &data[o]; o += (uint16_t)page_count * NFC_TAG_MF0_NTAG_DATA_SIZE;
    SKY_NEED(1); uint8_t has = data[o++];

    uint8_t *p_ver = NULL, *p_sig = NULL, *p_ctr = NULL, *p_nick = NULL;
    uint8_t nick_len = 0;
    if (has & SKY_HAS_VERSION)   { SKY_NEED(NFC_TAG_MF0_NTAG_VER_SIZE); p_ver = &data[o]; o += NFC_TAG_MF0_NTAG_VER_SIZE; }
    if (has & SKY_HAS_SIGNATURE) { SKY_NEED(NFC_TAG_MF0_NTAG_SIG_SIZE); p_sig = &data[o]; o += NFC_TAG_MF0_NTAG_SIG_SIZE; }
    if (has & SKY_HAS_COUNTER)   { SKY_NEED(3); p_ctr = &data[o]; o += 3; }
    if (has & SKY_HAS_NICK)      { SKY_NEED(1); nick_len = data[o++]; SKY_NEED(nick_len); p_nick = &data[o]; o += nick_len; }
    #undef SKY_NEED

    if (o != length) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);

    if (slot >= TAG_MAX_SLOT_NUM) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if (flags != 0) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if ((has & ~(SKY_HAS_VERSION | SKY_HAS_SIGNATURE | SKY_HAS_COUNTER | SKY_HAS_NICK)) != 0) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    if (!is_tag_specific_type_valid(tag_type)) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if (get_sense_type_from_tag_type(tag_type) != TAG_SENSE_HF) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if (get_buffer_by_tag_type(tag_type) == NULL) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if (!is_valid_uid_size(uid_len)) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if (ats_len > sizeof(((nfc_14a_ats_t *)0)->data)) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    if (nick_len > 32) return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    int nr_pages = nfc_tag_mf0_ntag_get_nr_pages_by_tag_type(tag_type);
    if (nr_pages <= 0 || page_count != (uint8_t)nr_pages) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }

    tag_mode_enter();
    change_slot_auto(slot, SKY_SLOT_CHANGE_BY_APP);
    tag_emulation_change_type(slot, tag_type);
    if (!nfc_tag_mf0_ntag_data_factory_ram(tag_type)) {
        return sky_write_failed(cmd, STATUS_FLASH_WRITE_FAIL, slot);
    }

    nfc_tag_14a_coll_res_reference_t *coll = get_coll_res_data(true);
    if (coll == NULL) {
        return sky_write_failed(cmd, STATUS_INVALID_SLOT_TYPE, slot);
    }
    *(coll->size) = (nfc_tag_14a_uid_size)uid_len;
    memcpy(coll->uid, uid, uid_len);
    memcpy(coll->atqa, atqa, 2);
    coll->sak[0] = sak;
    coll->ats->length = ats_len;
    if (ats_len > 0) memcpy(coll->ats->data, ats, ats_len);
    ...
    nfc_tag_mf0_ntag_information_t *info = (nfc_tag_mf0_ntag_information_t *)buffer->buffer;
    memcpy(&info->memory[0][0], pages, (size_t)page_count * NFC_TAG_MF0_NTAG_DATA_SIZE);

    if (p_ver != NULL) {
        uint8_t *dst = nfc_tag_mf0_ntag_get_version_data();
        if (dst == NULL) {
            return sky_write_failed(cmd, STATUS_INVALID_SLOT_TYPE, slot);
        }
        memcpy(dst, p_ver, NFC_TAG_MF0_NTAG_VER_SIZE);
    }
    if (p_sig != NULL) {
        uint8_t *dst = nfc_tag_mf0_ntag_get_signature_data();
        if (dst == NULL) {
            return sky_write_failed(cmd, STATUS_INVALID_SLOT_TYPE, slot);
        }
        memcpy(dst, p_sig, NFC_TAG_MF0_NTAG_SIG_SIZE);
    }
    if (p_ctr != NULL) {
        uint8_t *dst = nfc_tag_mf0_ntag_get_counter_data_by_index(0);
        if (dst == NULL) {
            return sky_write_failed(cmd, STATUS_INVALID_SLOT_TYPE, slot);
        }
        dst[MF0_NTAG_AUTHLIM_OFF_IN_CTR] &= ~MF0_NTAG_TEARING_MASK_IN_AUTHLIM;
        memcpy(dst, p_ctr, 3);
    }
    ...
    if (p_nick != NULL) {
        fds_slot_record_map_t map_info;
        get_fds_map_by_slot_sense_type_for_nick(slot, TAG_SENSE_HF, &map_info);
        uint8_t nick_buf[36] = { 0 };
        nick_buf[0] = nick_len;
        memcpy(nick_buf + 1, p_nick, nick_len);
        if (!fds_write_sync(map_info.id, map_info.key, sizeof(nick_buf), nick_buf)) {
            NRF_LOG_WARNING("sky_write_slot: nick write failed (non-fatal)");
        }
    }

    tag_emulation_sense_run();
    tag_emulation_slot_mark_written(slot);
...
static data_frame_tx_t *cmd_processor_sky_get_fds_stat(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    uint16_t valid = 0, dirty = 0, used = 0, freeable = 0, contig = 0, open_rec = 0;
    uint8_t pages = 0;
    bool corrupt = false;
    if (!fds_space_stat(&valid, &dirty, &used, &freeable, &contig, &open_rec, &pages, &corrupt)) {
        return data_frame_make(cmd, STATUS_NOT_IMPLEMENTED, 0, NULL);
    }
    uint8_t out[6 * 2 + 2 * 1 + 2];
    uint8_t *p = out;
    #define SKY_PUT16(v) do { *p++ = ((v) >> 8) & 0xFF; *p++ = (v) & 0xFF; } while (0)
    SKY_PUT16(valid);
    SKY_PUT16(dirty);
    SKY_PUT16(used);
    SKY_PUT16(freeable);
    ...
    *p++ = pages;
    *p++ = corrupt ? 1 : 0;
    SKY_PUT16((uint16_t)FDS_VIRTUAL_PAGE_SIZE);
    #undef SKY_PUT16
...
static data_frame_tx_t *cmd_processor_sky_get_slot_health(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    const uint8_t count = (uint8_t)TAG_MAX_SLOT_NUM;
    const uint8_t bytes = (uint8_t)((TAG_MAX_SLOT_NUM + 7u) / 8u);
    uint8_t out[2 + (TAG_MAX_SLOT_NUM + 7u) / 8u];
    out[0] = 1;
    out[1] = count;
    memset(out + 2, 0, bytes);
    for (uint8_t i = 0; i < count; i++) {
        if (tag_emulation_slot_data_missing(i)) {
            out[2 + (i >> 3)] |= (uint8_t)(1u << (i & 7u));
        }
    }
    return data_frame_make(cmd, STATUS_SUCCESS, (uint16_t)(2 + bytes), out);
}
...
static data_frame_tx_t *cmd_processor_sky_get_nfct_diag(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    nfc_14a_nfct_diag_t d;
    nfc_tag_14a_nfct_diag(&d);
    uint8_t out[1 + 11 + (6 + 2) * 2 + 2];
    uint8_t *p = out;
    *p++ = 3;
    *p++ = d.tag_state;
    *p++ = d.field_raw;
    ...
    *p++ = d.slot_state;
    *p++ = d.resume_armed;
    *p++ = d.recovery_pending;
    #define SKY_DIAG_PUT16(v) do { *p++ = ((v) >> 8) & 0xFF; *p++ = (v) & 0xFF; } while (0)
    SKY_DIAG_PUT16(d.ev_detected);
    SKY_DIAG_PUT16(d.ev_lost);
    SKY_DIAG_PUT16(d.ev_error);
    SKY_DIAG_PUT16(d.field_begin);
    SKY_DIAG_PUT16(d.recovery);
    SKY_DIAG_PUT16(d.idle_rearms);
    SKY_DIAG_PUT16(d.slotcut_in_field);
    SKY_DIAG_PUT16(d.tx_clamped);
    #undef SKY_DIAG_PUT16
    *p++ = d.field_events_filtered;
    *p++ = d.field_validation_pending;
    return data_frame_make(cmd, STATUS_SUCCESS, sizeof(out), out);
}
...
static data_frame_tx_t *cmd_processor_sky_get_nfc_stat(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    nfc_14a_nfc_stat_t st;
    nfc_tag_14a_nfc_stat(&st);
    uint8_t out[2 + 1 + 1 + 1 + 1 + 1];
    out[0] = (st.fixes >> 8) & 0xFF;
    out[1] = st.fixes & 0xFF;
    out[2] = st.rebuilds;
    out[3] = st.live;
    out[4] = st.sense;
    out[5] = st.field;
    out[6] = tag_emulation_slot_state();
    return data_frame_make(cmd, STATUS_SUCCESS, sizeof(out), out);
}
...
static data_frame_tx_t *cmd_processor_sky_get_power(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    uint8_t out[2 + 1 + 1];
    out[0] = (batt_lvl_in_milli_volts >> 8) & 0xFF;
    out[1] = batt_lvl_in_milli_volts & 0xFF;
    out[2] = percentage_batt_lvl;
    out[3] = ble_power_flags();
    return data_frame_make(cmd, STATUS_SUCCESS, sizeof(out), out);
}
...
static volatile uint16_t m_sky_evt_mask = 0;
static volatile uint16_t m_sky_evt_pending = 0;

static volatile uint8_t m_sky_evt_slot;
static volatile uint8_t m_sky_evt_slot_reason;
static volatile uint16_t m_sky_evt_batt_mv;
static volatile uint8_t m_sky_evt_batt_pct;
static volatile uint16_t m_sky_evt_pwr_mv;
static volatile uint8_t m_sky_evt_pwr_pct;
static volatile uint8_t m_sky_evt_pwr_flags;

void sky_event_set_mask(uint16_t mask) {
    CRITICAL_REGION_ENTER();
    m_sky_evt_mask = mask & SKY_EVT_MASK_ALL;
    m_sky_evt_pending &= m_sky_evt_mask;
    CRITICAL_REGION_EXIT();
}

uint16_t sky_event_get_mask(void) {
    return m_sky_evt_mask;
}
...
void sky_event_note_slot_changed(uint8_t slot, uint8_t reason) {
    CRITICAL_REGION_ENTER();
    m_sky_evt_slot = slot;
    m_sky_evt_slot_reason = reason;
    m_sky_evt_pending |= SKY_EVT_MASK_SLOT_CHANGED;
    CRITICAL_REGION_EXIT();
}

void sky_event_note_battery(uint16_t millivolt, uint8_t percent) {
    CRITICAL_REGION_ENTER();
    m_sky_evt_batt_mv = millivolt;
    m_sky_evt_batt_pct = percent;
    m_sky_evt_pending |= SKY_EVT_MASK_BATTERY;
    CRITICAL_REGION_EXIT();
}

void sky_event_note_power(uint16_t millivolt, uint8_t percent, uint8_t flags) {
    CRITICAL_REGION_ENTER();
    m_sky_evt_pwr_mv = millivolt;
    m_sky_evt_pwr_pct = percent;
    m_sky_evt_pwr_flags = flags;
    m_sky_evt_pending |= SKY_EVT_MASK_POWER;
    CRITICAL_REGION_EXIT();
}

static uint16_t sky_event_claim(uint8_t *out, uint16_t *out_len, uint16_t *out_cmd) {
    uint16_t claimed = 0;
    CRITICAL_REGION_ENTER();
    uint16_t ready = m_sky_evt_pending & m_sky_evt_mask;
    if (ready & SKY_EVT_MASK_SLOT_CHANGED) {
        claimed = SKY_EVT_MASK_SLOT_CHANGED;
        *out_cmd = DATA_CMD_SKY_EVT_SLOT_CHANGED;
        out[0] = m_sky_evt_slot;
        out[1] = m_sky_evt_slot_reason;
        *out_len = 2;
    } else if (ready & SKY_EVT_MASK_TAG_READ) {
        claimed = SKY_EVT_MASK_TAG_READ;
        *out_cmd = DATA_CMD_SKY_EVT_TAG_READ;
        *out_len = 0;
    } else if (ready & SKY_EVT_MASK_BATTERY) {
        claimed = SKY_EVT_MASK_BATTERY;
        *out_cmd = DATA_CMD_SKY_EVT_BATTERY;
        out[0] = (m_sky_evt_batt_mv >> 8) & 0xFF;
        out[1] = m_sky_evt_batt_mv & 0xFF;
        out[2] = m_sky_evt_batt_pct;
        *out_len = 3;
    } else if (ready & SKY_EVT_MASK_POWER) {
        claimed = SKY_EVT_MASK_POWER;
        *out_cmd = DATA_CMD_SKY_EVT_POWER;
        out[0] = (m_sky_evt_pwr_mv >> 8) & 0xFF;
        out[1] = m_sky_evt_pwr_mv & 0xFF;
        out[2] = m_sky_evt_pwr_pct;
        out[3] = m_sky_evt_pwr_flags;
        *out_len = 4;
    }
    m_sky_evt_pending &= (uint16_t)~claimed;
    CRITICAL_REGION_EXIT();
    return claimed;
}

void sky_event_process(void) {
    if (m_sky_evt_mask == 0) return;
    if (!is_usb_working() && !is_nus_working()) return;

    if (is_usb_working() && usb_cdc_tx_busy()) return;

    uint8_t payload[8];
    uint16_t len = 0;
    uint16_t cmd = 0;
    if (sky_event_claim(payload, &len, &cmd) == 0) return;

    data_frame_tx_t *frame = data_frame_make(cmd, STATUS_SUCCESS, len, len ? payload : NULL);
    if (frame != NULL) auto_response_data(frame);
}
...
static data_frame_tx_t *cmd_processor_sky_set_event_mask(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    if ((mask & ~SKY_EVT_MASK_ALL) != 0) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    sky_event_set_mask(mask);
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}

static data_frame_tx_t *cmd_processor_sky_get_ble_idle(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    const uint16_t seconds = settings_get_ble_idle_seconds();
    const uint16_t min     = SETTINGS_BLE_IDLE_MIN_S;
    const uint16_t max     = SETTINGS_BLE_IDLE_MAX_S;
    uint8_t resp[8];
    resp[0] = 1;
    resp[1] = (uint8_t)(seconds >> 8);
    resp[2] = (uint8_t)(seconds & 0xFFu);
    resp[3] = (uint8_t)(min >> 8);
    resp[4] = (uint8_t)(min & 0xFFu);
    resp[5] = (uint8_t)(max >> 8);
    resp[6] = (uint8_t)(max & 0xFFu);
    resp[7] = ble_idle_flags();
    return data_frame_make(cmd, STATUS_SUCCESS, sizeof(resp), resp);
}
...
static data_frame_tx_t *cmd_processor_sky_set_ble_idle(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    ...
    const uint16_t seconds = ((uint16_t)data[0] << 8) | (uint16_t)data[1];
    if (!settings_ble_idle_seconds_valid(seconds)) {
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
    }
    settings_set_ble_idle_seconds(seconds);
    ble_idle_apply_config();
    NRF_LOG_INFO("SKY: ble idle window -> %u s", (unsigned)seconds);
    uint8_t resp[3];
    resp[0] = 1;
    resp[1] = (uint8_t)(seconds >> 8);
    resp[2] = (uint8_t)(seconds & 0xFFu);
...
void on_data_frame_received(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    sky_cycle_note_command();
    ble_idle_note_activity();
    sky_cycle_note_host_command(cmd);
...
#define SKY_CYCLE_STATE_VERSION  1u
#define SKY_CYCLE_QUIET_MS       2000u

typedef struct ALIGN_U32 PACKED {
    uint8_t version;
    uint8_t enabled;
    uint8_t lap_done;
    uint8_t reserved;
...
static sky_cycle_state_t m_cycle;
static sky_cycle_state_t m_saved;
static bool m_saved_valid;
static uint16_t m_lap_target;
static bool m_cmd_seen;
static uint32_t m_last_cmd_ticks;
static bool m_enter_pending;

static volatile bool m_read_pending;
static volatile uint8_t m_read_slot;
static volatile bool m_advance_pending;
static volatile bool m_light_fix;
static volatile bool m_flush_pending;


static bool sky_cycle_slot_readable(uint8_t slot) {
    if (!is_slot_enabled(slot, TAG_SENSE_HF)) return false;
    if (tag_emulation_slot_data_missing(slot)) return false;
    tag_slot_specific_type_t types;
    tag_emulation_get_specific_types_by_slot(slot, &types);
    return nfc_tag_mf0_ntag_get_nr_pages_by_tag_type(types.tag_hf) > 0;
}

static uint16_t sky_cycle_count_readable(void) {
    uint16_t n = 0;
    for (uint8_t i = 0; i < TAG_MAX_SLOT_NUM; i++) {
        if (sky_cycle_slot_readable(i)) n++;
    }
    return n;
}

static uint8_t sky_cycle_first_readable(void) {
    for (uint8_t i = 0; i < TAG_MAX_SLOT_NUM; i++) {
        if (sky_cycle_slot_readable(i)) return i;
    }
    return tag_emulation_get_slot();
}

static uint8_t sky_cycle_find_next(uint8_t from) {
    for (uint16_t step = 1; step < TAG_MAX_SLOT_NUM; step++) {
        uint8_t s = (uint8_t)((from + step) % TAG_MAX_SLOT_NUM);
        if (sky_cycle_slot_readable(s)) return s;
    }
    return from;
}


static bool sky_cycle_quiet_elapsed(void) {
    if (!m_cmd_seen) return true;
    uint32_t diff = app_timer_cnt_diff_compute(app_timer_cnt_get(), m_last_cmd_ticks);
    return diff > APP_TIMER_TICKS(SKY_CYCLE_QUIET_MS);
}


uint8_t sky_cycle_led_color(uint8_t slot) {
    return m_cycle.enabled ? (uint8_t)RGB_WHITE : get_color_by_slot(slot);
}

void sky_cycle_apply_idle_light(void) {
    uint8_t slot = tag_emulation_get_slot();
    set_slot_light_color(sky_cycle_led_color(slot));
    light_up_by_slot();
}

void sky_cycle_apply_light_preserving(void) {
    if (m_cycle.enabled) set_slot_light_color(RGB_WHITE);
    light_up_by_slot();
}

static void sky_cycle_reject_blink(void) {
    rgb_marquee_stop();
    uint32_t *led_pins = hw_get_led_array();
    set_slot_light_color(RGB_RED);
    for (uint8_t i = 0; i < 2; i++) {
        for (uint8_t j = 0; j < RGB_LIST_NUM; j++) nrf_gpio_pin_set(led_pins[j]);
        bsp_delay_ms(120);
        for (uint8_t j = 0; j < RGB_LIST_NUM; j++) nrf_gpio_pin_clear(led_pins[j]);
        bsp_delay_ms(120);
    }
    sky_cycle_apply_idle_light();
}


void sky_cycle_save(void) {
    sky_cycle_state_t copy = m_cycle;
    copy.version = (uint8_t)SKY_CYCLE_STATE_VERSION;
    if (m_saved_valid && memcmp(&copy, &m_saved, sizeof(copy)) == 0) return;
    if (!fds_write_sync(FDS_SKY_CYCLE_FILE_ID, FDS_SKY_CYCLE_RECORD_KEY, sizeof(copy), &copy)) {
        NRF_LOG_WARNING("sky_cycle: save failed (state kept in RAM)");
        return;
    }
    m_saved = copy;
    m_saved_valid = true;
...
bool sky_cycle_is_enabled(void) {
    return m_cycle.enabled != 0;
}

void sky_cycle_init(void) {
    uint16_t len = sizeof(m_cycle);
    bool ok = fds_read_sync(FDS_SKY_CYCLE_FILE_ID, FDS_SKY_CYCLE_RECORD_KEY, &len, (uint8_t *)&m_cycle);
    if (!ok || m_cycle.version != (uint8_t)SKY_CYCLE_STATE_VERSION) {
        m_cycle.version = (uint8_t)SKY_CYCLE_STATE_VERSION;
        m_cycle.enabled = 0;
        m_cycle.lap_done = 0;
        m_cycle.reserved = 0;
        m_lap_target = 0;
    ...
    m_cycle.enabled = m_cycle.enabled ? 1 : 0;
    m_saved = m_cycle;
    m_saved_valid = true;
    m_lap_target = sky_cycle_count_readable();
    if (m_cycle.enabled && m_lap_target == 0) {
        m_cycle.enabled = 0;
        m_cycle.lap_done = 0;
    }
    NRF_LOG_INFO("sky_cycle: restored enabled=%d lap=%d/%d", m_cycle.enabled, m_cycle.lap_done, m_lap_target);
}


static bool sky_cycle_exit_path_ok(void) {
    if (settings_get_long_button_press_config('a') == SettingsButtonToggleAutoAdvance &&
        settings_get_button_press_config('a') != SettingsButtonDisable) return true;
    if (settings_get_long_button_press_config('b') == SettingsButtonToggleAutoAdvance &&
        settings_get_button_press_config('b') != SettingsButtonDisable) return true;
    if (settings_get_button_press_config('a') != SettingsButtonDisable) return true;
    if (settings_get_button_press_config('b') != SettingsButtonDisable) return true;
    return false;
}

#define SKY_CYCLE_OFF_TOGGLE        0
#define SKY_CYCLE_OFF_SHORT_PRESS   1
#define SKY_CYCLE_OFF_HOST_CMD      2
#define SKY_CYCLE_OFF_SLOT_TAKEOVER 3

static void sky_cycle_exit(uint8_t why) {
    if (!m_cycle.enabled) return;
    m_cycle.enabled = 0;
    m_cycle.lap_done = 0;
    CRITICAL_REGION_ENTER();
    m_read_pending = false;
    m_advance_pending = false;
    CRITICAL_REGION_EXIT();
    m_enter_pending = false;
    m_light_fix = true;
    NRF_LOG_INFO("sky_cycle: OFF (reason=%d)", why);
    sky_cycle_save();
}

static void sky_cycle_set(bool on) {
    if (on) {
        if (!sky_cycle_exit_path_ok()) {
            NRF_LOG_WARNING("sky_cycle: refuse to enter (no exit path)");
            sky_cycle_reject_blink();
            return;
        }
        uint16_t readable = sky_cycle_count_readable();
        if (readable == 0) {
            NRF_LOG_WARNING("sky_cycle: refuse to enter (no readable slot)");
            sky_cycle_reject_blink();
            return;
        }
        m_cycle.enabled = 1;
        m_cycle.lap_done = 0;
        m_lap_target = readable;
        m_light_fix = true;
        NRF_LOG_INFO("sky_cycle: ON (readable=%d)", readable);
        sky_cycle_save();
        m_enter_pending = true;
    } else {
        sky_cycle_exit(SKY_CYCLE_OFF_TOGGLE);
        sky_cycle_apply_idle_light();
    }
}

void sky_cycle_toggle(void) {
    sky_cycle_set(!m_cycle.enabled);
}

void sky_cycle_note_short_press(char which) {
    if (!m_cycle.enabled) return;
    if (settings_get_button_press_config(which) == SettingsButtonToggleAutoAdvance) return;
    sky_cycle_exit(SKY_CYCLE_OFF_SHORT_PRESS);
}

void sky_cycle_note_slot_taken_over(void) {
    sky_cycle_exit(SKY_CYCLE_OFF_SLOT_TAKEOVER);
}
...
void sky_cycle_note_field_lost(void) {
    ...
    uint8_t read_slot;
    CRITICAL_REGION_ENTER();
    had_read = m_read_pending;
    read_slot = m_read_slot;
    m_read_pending = false;
    CRITICAL_REGION_EXIT();

    if (m_cycle.enabled) m_light_fix = true;
    if (had_read && m_cycle.enabled) {
        m_advance_pending = true;
        NRF_LOG_INFO("sky_cycle: read slot %d -> advance armed", read_slot);
    }
}
...
void sky_cycle_note_command(void) {
    m_last_cmd_ticks = app_timer_cnt_get();
    m_cmd_seen = true;
}

static bool sky_cycle_is_host_slot_command(uint16_t cmd) {
    switch (cmd) {
        ...
        case DATA_CMD_SKY_ACTIVATE_SLOT:
        case DATA_CMD_SKY_WRITE_SLOT:
            return true;
        default:
            return false;
    }
...
void sky_cycle_note_host_command(uint16_t cmd) {
    if (!m_cycle.enabled) return;
    if (!sky_cycle_is_host_slot_command(cmd)) return;
    sky_cycle_exit(SKY_CYCLE_OFF_HOST_CMD);
}
...
void sky_cycle_process(void) {
    bool busy = rgb_marquee_slot_switch_busy();
    if (m_light_fix && !busy && !g_is_tag_emulating) {
        m_light_fix = false;
        sky_cycle_apply_idle_light();
    }

    if (m_flush_pending && !is_nus_working() && !g_is_tag_emulating && !busy) {
    ...
    bool need_home = m_enter_pending;
    if (!need_home && !sky_cycle_slot_readable(tag_emulation_get_slot())) {
        if (!g_is_tag_emulating && get_device_mode() == DEVICE_MODE_TAG &&
            sky_cycle_count_readable() == 0) {
            NRF_LOG_WARNING("sky_cycle: no readable slot -> auto OFF");
            sky_cycle_set(false);
            return;
        }
        need_home = true;
    }
    if (need_home && !g_is_tag_emulating && !nfc_tag_14a_field_present() &&
        get_device_mode() == DEVICE_MODE_TAG &&
        sky_cycle_quiet_elapsed()) {
        m_enter_pending = false;
        CRITICAL_REGION_ENTER();
        m_advance_pending = false;
        CRITICAL_REGION_EXIT();
        uint8_t first = sky_cycle_first_readable();
        if (first != tag_emulation_get_slot()) {
            change_slot_auto(first, SKY_SLOT_CHANGE_BY_AUTO);
        } else {
            sky_cycle_apply_idle_light();
        }
    } else if (need_home) {
    ...
    bool armed;
    CRITICAL_REGION_ENTER();
    armed = m_advance_pending;
    m_advance_pending = false;
    CRITICAL_REGION_EXIT();
    if (!armed) return;
    if (g_is_tag_emulating || nfc_tag_14a_field_present()) { m_advance_pending = true; return; }
    if (get_device_mode() != DEVICE_MODE_TAG) { m_advance_pending = true; return; }
    if (!sky_cycle_quiet_elapsed()) { m_advance_pending = true; return; }
    uint8_t cur = tag_emulation_get_slot();
    uint8_t next = sky_cycle_find_next(cur);
    if (next == cur) {
        sky_cycle_set(false);
        return;
    }
    m_cycle.lap_done++;
    change_slot_auto(next, SKY_SLOT_CHANGE_BY_AUTO);
    if (m_cycle.lap_done >= m_lap_target) {
        NRF_LOG_INFO("sky_cycle: lap done (%d) -> auto OFF", m_cycle.lap_done);
        sky_cycle_set(false);
    }
}
