...
void light_up_by_slot(void) {
    ...
    uint8_t led = led_index_of_slot(tag_emulation_get_slot());
...
void apply_slot_change(uint8_t slot_now, uint8_t slot_new) {
    if (slot_now == slot_new || settings_get_animation_config() == SettingsAnimationModeNone) {
        sky_cycle_apply_light_preserving();
        return;
    }
    uint8_t color_now = sky_cycle_led_color(slot_now);
    uint8_t color_new = sky_cycle_led_color(slot_new);
    rgb_marquee_slot_switch(led_index_of_slot(slot_now), color_now,
                            led_index_of_slot(slot_new), color_new);
}
