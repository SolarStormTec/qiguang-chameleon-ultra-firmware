...
static bool m_hf_data_ready = false;
...
#define TAG_SLOT_DAMAGED_WORDS  ((TAG_MAX_SLOT_NUM + 7u) / 8u)
static uint8_t m_slot_data_missing[TAG_SLOT_DAMAGED_WORDS];

static void slot_damaged_set(uint8_t slot) {
    if (slot >= TAG_MAX_SLOT_NUM) return;
    m_slot_data_missing[slot >> 3] |= (uint8_t)(1u << (slot & 7u));
}

static void slot_damaged_clear(uint8_t slot) {
    if (slot >= TAG_MAX_SLOT_NUM) return;
    m_slot_data_missing[slot >> 3] &= (uint8_t)~(1u << (slot & 7u));
}

bool tag_emulation_slot_data_missing(uint8_t slot) {
    if (slot >= TAG_MAX_SLOT_NUM) return false;
    return (m_slot_data_missing[slot >> 3] & (uint8_t)(1u << (slot & 7u))) != 0;
}

void tag_emulation_slot_mark_written(uint8_t slot) {
    slot_damaged_clear(slot);
}

uint8_t tag_emulation_slot_state(void) {
    uint8_t slot = tag_emulation_get_slot();
    tag_slot_specific_type_t types;
    tag_emulation_get_specific_types_by_slot(slot, &types);
    if (types.tag_hf == TAG_TYPE_UNDEFINED) return 0;
    return tag_emulation_slot_data_missing(slot) ? 2 : 1;
}
...
bool tag_emulation_load_by_buffer(tag_specific_type_t tag_type, bool update_crc) {
    ...
    if (length > 0 && get_sense_type_from_tag_type(tag_type) == TAG_SENSE_HF) {
        m_hf_data_ready = true;
    }
...
static void load_data_by_tag_type(uint8_t slot, tag_specific_type_t tag_type) {
    if (get_sense_type_from_tag_type(tag_type) == TAG_SENSE_HF && slot == tag_emulation_get_slot()) {
        m_hf_data_ready = false;
    ...
    if (false == ret) {
        if (sense_type == TAG_SENSE_HF) slot_damaged_set(slot);
        NRF_LOG_WARNING("tag slot %d has metadata but no data; marked damaged", slot);
        return;
    }
    if (sense_type == TAG_SENSE_HF) slot_damaged_clear(slot);
 ...
 */
static bool save_data_by_tag_type(uint8_t slot, tag_specific_type_t tag_type) {
...
bool tag_emulation_sense_should_be_on(void) {
    uint8_t slot = tag_emulation_get_slot();
    tag_slot_specific_type_t types;
    tag_emulation_get_specific_types_by_slot(slot, &types);
    return slotConfig.slots[slot].enabled_hf && (types.tag_hf != TAG_TYPE_UNDEFINED);
}

void tag_emulation_sense_rebuild(void) {
...
bool tag_emulation_save_data(void) {
    uint8_t slot = tag_emulation_get_slot();
    bool hf_ok = save_data_by_tag_type(slot, slotConfig.slots[slot].tag_hf);
    bool lf_ok = save_data_by_tag_type(slot, slotConfig.slots[slot].tag_lf);
    return hf_ok && lf_ok;
}
...
static void tag_emulation_sense_switch_all(bool enable) {
    ...
    if (enable && m_hf_data_ready && (slotConfig.slots[slot].enabled_hf) && (slotConfig.slots[slot].tag_hf != TAG_TYPE_UNDEFINED)) {
...
void tag_emulation_sense_switch(tag_sense_type_t type, bool enable) {
    ...
    switch (type) {
        case TAG_SENSE_NO:
            ...
            break;
        case TAG_SENSE_HF:
            if (enable && m_hf_data_ready && (slotConfig.slots[slot].enabled_hf) &&
                    (slotConfig.slots[slot].tag_hf != TAG_TYPE_UNDEFINED)) {
                nfc_tag_14a_sense_switch(true);
            } else {
                nfc_tag_14a_sense_switch(false);
...
#define TAG_SLOT_CONFIG_V0_SLOT_NUM  8
#define TAG_SLOT_CONFIG_V0_SIZE      (4 + TAG_SLOT_CONFIG_V0_SLOT_NUM * 4)

static void tag_emulation_migrate_slot_config_v0_to_v8(void) {
    // Copy old slotConfig content
    uint8_t tmpbuf[TAG_SLOT_CONFIG_V0_SIZE];
    ...
    for (uint8_t i = 0; i < MIN(TAG_SLOT_CONFIG_V0_SLOT_NUM, ARRAYLEN(slotConfig.slots)); i++) {
...
static void tag_emulation_migrate_slot_config_v8_to_v9(void) {
    NRF_LOG_INFO("Migrating slotConfig v8 -> v9 (8 -> %d slots)", TAG_MAX_SLOT_NUM);
    for (uint8_t i = TAG_SLOT_CONFIG_V0_SLOT_NUM; i < ARRAYLEN(slotConfig.slots); i++) {
        slotConfig.slots[i].enabled_hf = false;
        slotConfig.slots[i].enabled_lf = false;
        slotConfig.slots[i].tag_hf = TAG_TYPE_UNDEFINED;
        slotConfig.slots[i].tag_lf = TAG_TYPE_UNDEFINED;
    }
    if (slotConfig.active_slot >= ARRAYLEN(slotConfig.slots)) {
        NRF_LOG_WARNING("slotConfig.active_slot %d out of range, clamped to 0", slotConfig.active_slot);
        slotConfig.active_slot = 0;
    }
    slotConfig.version = TAG_SLOT_CONFIG_CURRENT_VERSION;
}

#define TAG_SLOT_CONFIG_V9_SLOT_NUM  128

static void tag_emulation_migrate_slot_config_v9_to_v10(void) {
    NRF_LOG_INFO("Migrating slotConfig v9 -> v10 (%d -> %d slots)", TAG_SLOT_CONFIG_V9_SLOT_NUM, TAG_MAX_SLOT_NUM);
    for (uint16_t i = TAG_SLOT_CONFIG_V9_SLOT_NUM; i < ARRAYLEN(slotConfig.slots); i++) {
        slotConfig.slots[i].enabled_hf = false;
        slotConfig.slots[i].enabled_lf = false;
        slotConfig.slots[i].tag_hf = TAG_TYPE_UNDEFINED;
        slotConfig.slots[i].tag_lf = TAG_TYPE_UNDEFINED;
...
bool tag_emulation_save(void) {
    bool config_ok = tag_emulation_save_config();
    if (!config_ok) return false;
    return tag_emulation_save_data();
}
...
void tag_emulation_change_type(uint8_t slot, tag_specific_type_t tag_type) {
    ...
    if (sense_type != TAG_SENSE_NO && tag_emulation_get_slot() == slot) {
        load_data_by_tag_type(slot, tag_type);
