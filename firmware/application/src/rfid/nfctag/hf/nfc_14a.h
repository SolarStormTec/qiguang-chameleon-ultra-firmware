...
#define NFC_14A_TX_MAX_BYTES    511u
...
typedef struct {
    ...
    uint8_t  slot_state;
    uint8_t  resume_armed;
    uint8_t  recovery_pending;
    uint16_t ev_detected;
    uint16_t ev_lost;
    uint16_t ev_error;
    uint16_t field_begin;
    uint16_t recovery;
    uint16_t idle_rearms;
    uint16_t slotcut_in_field;
    uint16_t tx_clamped;
    uint8_t field_events_filtered;
    uint8_t field_validation_pending;
} nfc_14a_nfct_diag_t;
