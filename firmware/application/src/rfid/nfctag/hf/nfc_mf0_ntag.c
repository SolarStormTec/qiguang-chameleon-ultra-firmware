...
static void handle_any_read(uint8_t block_num, uint8_t block_cnt, uint8_t block_max) {
            ...
            if ((pages_needed >= user_data_end) || ((user_data_end - pages_needed) < mirror_page_off)) {
                ...
                switch (mirror_mode) {
                    ...
                    case MIRROR_CONF_UID_CNT:
                        bytes2hex(m_tag_information->res_coll.uid, (char *)mirror_buf, 7);
                        mirror_buf[14] = 'x';
                        bytes2hex(get_counter_data_by_index(0, false), (char *)&mirror_buf[15], 3);
                        break;
                }
            }
        }
    ...
    if (((uint32_t)block_cnt * NFC_TAG_MF0_NTAG_DATA_SIZE) > NFC_14A_TX_MAX_BYTES) {
        nfc_tag_14a_tx_nbit(NAK_INVALID_OPERATION_TBV, 4);
        return;
    }

    nfc_tag_14a_note_ntag_read_tx();
...
static void mf0_ntag_fill_factory(nfc_tag_mf0_ntag_information_t *p_ntag_information, tag_specific_type_t tag_type) {
    // default ntag data
...
bool nfc_tag_mf0_ntag_data_factory_ram(tag_specific_type_t tag_type) {
    tag_data_buffer_t *buffer = get_buffer_by_tag_type(tag_type);
    if (buffer == NULL || buffer->length < sizeof(nfc_tag_mf0_ntag_information_max_t)) {
        return false;
    }
    mf0_ntag_fill_factory((nfc_tag_mf0_ntag_information_t *)buffer->buffer, tag_type);
    return tag_emulation_load_by_buffer(tag_type, true);
}
...
bool nfc_tag_mf0_ntag_data_factory(uint8_t slot, tag_specific_type_t tag_type) {
    ...
    nfc_tag_mf0_ntag_information_t *p_ntag_information = (nfc_tag_mf0_ntag_information_t *)&ntag_tmp_information;
    mf0_ntag_fill_factory(p_ntag_information, tag_type);
