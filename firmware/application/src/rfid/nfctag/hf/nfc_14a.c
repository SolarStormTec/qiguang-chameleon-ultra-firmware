...
static volatile bool m_ntag_read_tx_pending = false;

void nfc_tag_14a_note_ntag_read_tx(void) {
    m_ntag_read_tx_pending = true;
}
// Receiving buffer
...
static volatile bool m_session_recovery_pending = false;
#define NFC_FIELD_SETTLE_MS 5u
#define NFC_FIELD_RESUME_SETTLE_MS 5u
static volatile bool m_field_resume_armed = false;
static uint32_t m_field_resume_start = 0;
static volatile bool m_resume_mid_session = false;
static volatile uint32_t m_field_begin_ticks = 0;

static volatile bool m_hf_session_live = false;
static bool     m_reconcile_bad = false;
static uint32_t m_reconcile_bad_since = 0;
static bool     m_rebuild_seen = false;
static uint32_t m_last_rebuild_ticks = 0;
static volatile uint16_t m_reconcile_fixes = 0;
static volatile uint8_t m_reconcile_rebuilds = 0;

#define NFC_IDLE_REARM_MS 60000u

static uint32_t m_last_radio_activity_ticks = 0;
static bool     m_radio_activity_seen = false;
static volatile uint16_t m_diag_idle_rearms = 0;
static volatile uint16_t m_diag_ev_detected = 0;
static volatile uint16_t m_diag_ev_lost = 0;
static volatile uint16_t m_diag_ev_error = 0;
static volatile uint16_t m_diag_field_begin = 0;
static volatile uint16_t m_diag_recovery = 0;
static volatile uint16_t m_diag_slotcut_in_field = 0;
static volatile uint16_t m_diag_tx_clamped = 0;

void nfc_tag_14a_note_slotcut_in_field(void) {
    if (m_diag_slotcut_in_field < 0xFFFF) m_diag_slotcut_in_field++;
}

static inline void nfc_tag_14a_note_radio_activity(void) {
    m_last_radio_activity_ticks = app_timer_cnt_get();
    m_radio_activity_seen = true;
}

extern bool m_is_field_on;
extern bool g_is_reader_antenna_on;
 ...
 */
STATIC_ASSERT(NFC_14A_TX_MAX_BYTES ==
              (NFCT_TXD_AMOUNT_TXDATABYTES_Msk >> NFCT_TXD_AMOUNT_TXDATABYTES_Pos));
STATIC_ASSERT(NFC_14A_TX_MAX_BYTES <= MAX_NFC_TX_BUFFER_SIZE);

#define NFC_14A_TX_BYTE_CORE(data, bytes, appendCrc, delayMode)                                                  \
    do {                                                                                                         \
 \
        uint32_t tx_len = (bytes);                                                                               \
        if (tx_len > NFC_14A_TX_MAX_BYTES) {                                                                     \
            tx_len = NFC_14A_TX_MAX_BYTES;                                                                       \
            if (m_diag_tx_clamped < 0xFFFF) m_diag_tx_clamped++;                                                 \
        }                                                                                                        \
        m_is_responded = true;                                                                                   \
        memcpy(m_nfc_tx_buffer, data, tx_len);                                                                   \
        NRF_NFCT->PACKETPTR = (uint32_t)(m_nfc_tx_buffer);                                                       \
        NRF_NFCT->TXD.AMOUNT = (tx_len << NFCT_TXD_AMOUNT_TXDATABYTES_Pos) & NFCT_TXD_AMOUNT_TXDATABYTES_Msk;    \
        ...
        case NFC_TAG_STATE_14A_ACTIVE: {
            ...
            if (szDataBits == 32) {
                ...
                if (p_data[0] == NFC_TAG_14A_CMD_RATS && nfc_tag_14a_checks_crc(p_data, 4)) {
                    ...
                    if (auto_coll_res != NULL && auto_coll_res->ats != NULL && auto_coll_res->ats->length > 0) {
...
static void nfc_tag_14a_field_begin(void) {
    m_field_begin_ticks = app_timer_cnt_get();
    if (m_diag_field_begin < 0xFFFF) m_diag_field_begin++;
    nfc_tag_14a_note_radio_activity();

    if (!g_is_tag_emulating) {
        sleep_timer_stop();

        g_is_tag_emulating = true;
        m_hf_session_live = true;
        g_usb_led_marquee_enable = false;

        set_slot_light_color(RGB_GREEN);
        TAG_FIELD_LED_ON()

        sky_cycle_note_field_detected();
        ble_idle_note_activity();
    }

    NRF_LOG_INFO("HF FIELD BEGIN");
...
static bool nfc_tag_14a_field_present_raw(void) {
    return (nrf_nfct_field_status_get() & NRF_NFCT_FIELD_STATE_PRESENT_MASK) != 0;
}

bool nfc_tag_14a_field_present(void) {
    if (m_nfc_sense_state != NFC_SENSE_STATE_ENABLE) {
        return false;
    }
    return nfc_tag_14a_field_present_raw();
}

void nfc_tag_14a_field_resume_process(void) {
    if (!m_field_resume_armed) {
        return;
    }
    if (m_nfc_sense_state != NFC_SENSE_STATE_ENABLE || get_device_mode() != DEVICE_MODE_TAG) {
        m_field_resume_armed = false;
        return;
    }
    if (app_timer_cnt_diff_compute(app_timer_cnt_get(), m_field_resume_start) <
        APP_TIMER_TICKS(NFC_FIELD_RESUME_SETTLE_MS)) {
        return;
    }
    ...
    if (g_is_tag_emulating && !m_resume_mid_session) {
        return;
    }
    if (m_resume_mid_session) {
        g_is_tag_emulating = false;
    }
    nfc_tag_14a_field_begin();
}

#define NFC_RECONCILE_CONFIRM_MS  200u
#define NFC_REBUILD_MIN_GAP_MS   2000u

static void nfc_tag_14a_hf_session_close(void) {
    if (!m_hf_session_live) return;
    m_hf_session_live = false;
    if (!m_is_field_on && !g_is_reader_antenna_on) TAG_FIELD_LED_OFF();
    g_is_tag_emulating = false;
}

void nfc_tag_14a_field_reconcile(void) {
    if (get_device_mode() != DEVICE_MODE_TAG) {
        m_reconcile_bad = false;
        return;
    }

    if (m_nfc_sense_state != NFC_SENSE_STATE_ENABLE) {
        nfc_tag_14a_hf_session_close();
        m_reconcile_bad = false;

        if (!g_is_tag_emulating && tag_emulation_sense_should_be_on() &&
            !tag_emulation_slot_data_missing(tag_emulation_get_slot())) {
            uint32_t now = app_timer_cnt_get();
            if (!m_rebuild_seen ||
                app_timer_cnt_diff_compute(now, m_last_rebuild_ticks) >= APP_TIMER_TICKS(NFC_REBUILD_MIN_GAP_MS)) {
                m_rebuild_seen = true;
                m_last_rebuild_ticks = now;
                m_reconcile_fixes++;
                if (m_reconcile_rebuilds < 0xFF) m_reconcile_rebuilds++;
                NRF_LOG_WARNING("HF reconcile #%u: slot claims a card but sensing off -> rebuild",
                                (unsigned)m_reconcile_fixes);
                tag_emulation_sense_rebuild();
            }
        }
        return;
    }

    if (!m_radio_activity_seen) {
        nfc_tag_14a_note_radio_activity();
    } else if (!g_is_tag_emulating && !nfc_tag_14a_field_present_raw() &&
               tag_emulation_sense_should_be_on() &&
               !tag_emulation_slot_data_missing(tag_emulation_get_slot()) &&
               app_timer_cnt_diff_compute(app_timer_cnt_get(), m_last_radio_activity_ticks) >=
                   APP_TIMER_TICKS(NFC_IDLE_REARM_MS)) {
        if (m_diag_idle_rearms < 0xFFFF) m_diag_idle_rearms++;
        nfc_tag_14a_note_radio_activity();
        NRF_LOG_INFO("HF idle re-arm #%u (no field event for %us)",
                     (unsigned)m_diag_idle_rearms, (unsigned)(NFC_IDLE_REARM_MS / 1000u));
        m_session_recovery_pending = true;
    }

    bool field = nfc_tag_14a_field_present_raw();
    if (m_hf_session_live == field) {
        m_reconcile_bad = false;
        return;
    }
    ...
    uint32_t now = app_timer_cnt_get();
    if (!m_reconcile_bad) {
        m_reconcile_bad = true;
        m_reconcile_bad_since = now;
        return;
    }
    if (app_timer_cnt_diff_compute(now, m_reconcile_bad_since) < APP_TIMER_TICKS(NFC_RECONCILE_CONFIRM_MS)) {
        return;
    }
    m_reconcile_bad = false;

    if (nfc_tag_14a_field_present_raw() != field) return;

    m_reconcile_fixes++;
    if (field) {
        NRF_LOG_WARNING("HF reconcile #%u: field present but no session -> begin", (unsigned)m_reconcile_fixes);
        nfc_tag_14a_field_begin();
    } else {
        NRF_LOG_WARNING("HF reconcile #%u: session stale, field absent -> close + recovery",
                        (unsigned)m_reconcile_fixes);
        nfc_tag_14a_hf_session_close();
        m_session_recovery_pending = true;
    }
...
void nfc_tag_14a_nfc_stat(nfc_14a_nfc_stat_t *out) {
    out->fixes    = m_reconcile_fixes;
    out->rebuilds = m_reconcile_rebuilds;
    out->live     = m_hf_session_live ? 1 : 0;
    out->sense    = (uint8_t)m_nfc_sense_state;
    out->field    = nfc_tag_14a_field_present() ? 1 : 0;
}

void nfc_tag_14a_nfct_diag(nfc_14a_nfct_diag_t *out) {
    uint32_t inten = NRF_NFCT->INTENSET;

    out->tag_state  = (uint8_t)((NRF_NFCT->NFCTAGSTATE & NFCT_NFCTAGSTATE_NFCTAGSTATE_Msk) >>
                                NFCT_NFCTAGSTATE_NFCTAGSTATE_Pos);
    out->field_raw  = nrf_nfct_field_status_get();
    out->inten      = 0;
    if (inten & NRF_NFCT_INT_FIELDDETECTED_MASK) out->inten |= 0x01;
    if (inten & NRF_NFCT_INT_FIELDLOST_MASK)     out->inten |= 0x02;
    if (inten & NRF_NFCT_INT_ERROR_MASK)         out->inten |= 0x04;
    if (inten & NRF_NFCT_INT_SELECTED_MASK)      out->inten |= 0x08;
    out->err_status = (uint8_t)(NRF_NFCT->ERRORSTATUS & 0xFF);
    out->sense      = (uint8_t)m_nfc_sense_state;
    out->live       = m_hf_session_live ? 1 : 0;
    out->emulating  = g_is_tag_emulating ? 1 : 0;
    out->should_be_on = tag_emulation_sense_should_be_on() ? 1 : 0;
    out->slot_state = tag_emulation_slot_state();
    out->resume_armed = m_field_resume_armed ? 1 : 0;
    out->recovery_pending = m_session_recovery_pending ? 1 : 0;
    out->ev_detected = m_diag_ev_detected;
    out->ev_lost     = m_diag_ev_lost;
    out->ev_error    = m_diag_ev_error;
    out->field_begin = m_diag_field_begin;
    out->recovery    = m_diag_recovery;
    out->idle_rearms = m_diag_idle_rearms;
    out->slotcut_in_field = m_diag_slotcut_in_field;
    out->tx_clamped       = m_diag_tx_clamped;
    out->field_events_filtered    = nrfx_nfct_field_events_filtered() ? 1 : 0;
    out->field_validation_pending = nrfx_nfct_field_validation_pending() ? 1 : 0;
}

void nfc_tag_14a_event_callback(nrfx_nfct_evt_t const *p_event) {
    switch (p_event->evt_id) {
        case NRFX_NFCT_EVT_FIELD_DETECTED: {
            if (m_diag_ev_detected < 0xFFFF) m_diag_ev_detected++;
            nfc_tag_14a_note_radio_activity();
            nfc_tag_14a_field_begin();
            break;
        }
        case NRFX_NFCT_EVT_FIELD_LOST: {
            if (m_diag_ev_lost < 0xFFFF) m_diag_ev_lost++;
            nfc_tag_14a_note_radio_activity();
            m_ntag_read_tx_pending = false;
            g_is_tag_emulating = false;
            m_hf_session_live = false;
            // call sleep_timer_start *after* unsetting g_is_tag_emulating
            sleep_timer_start(sky_cycle_is_enabled() ? SKY_CYCLE_IDLE_MS : SLEEP_DELAY_MS_FIELD_NFC_LOST);

            sky_cycle_note_field_lost();
        ...
        case NRFX_NFCT_EVT_TX_FRAMEEND: {
            if (m_ntag_read_tx_pending) {
                m_ntag_read_tx_pending = false;
                sky_cycle_note_read(tag_emulation_get_slot());
            }
            // NRF_LOG_INFO("TX end.\n");
        ...
        case NRFX_NFCT_EVT_ERROR: {
            if (m_diag_ev_error < 0xFFFF) m_diag_ev_error++;
            nfc_tag_14a_note_radio_activity();
            m_ntag_read_tx_pending = false;
...
void nfc_tag_14a_set_handler(nfc_tag_14a_handler_t *handler) {
    if (handler != NULL) {
        ...
        m_tag_handler.cb_reset = NULL;
        m_tag_handler.cb_state = NULL;
        m_tag_handler.get_coll_res = NULL;
    }
}

static void nfc_tag_14a_field_settle_before_teardown(void) {
    for (uint8_t i = 0; i < NFC_FIELD_SETTLE_MS; i++) {
        if (app_timer_cnt_diff_compute(app_timer_cnt_get(), m_field_begin_ticks) >=
            APP_TIMER_TICKS(NFC_FIELD_SETTLE_MS)) {
            return;
        }
        bsp_delay_ms(1);
    }
    ...
    if (m_nfc_sense_state == NFC_SENSE_STATE_NONE || m_nfc_sense_state == NFC_SENSE_STATE_DISABLE) {
        if (enable) {
            ...
            if (nrfx_nfct_init(&nnct) != NRFX_SUCCESS) {
                NRF_LOG_WARNING("Cannot setup NFC! retry from clean state");
                nfc_tag_14a_field_settle_before_teardown();
                nrfx_nfct_uninit();
                if (nrfx_nfct_init(&nnct) != NRFX_SUCCESS) {
                    NRF_LOG_ERROR("Cannot setup NFC! sensing stays off");
            ...
            m_resume_mid_session = g_is_tag_emulating;
            m_field_resume_start = app_timer_cnt_get();
            m_field_resume_armed = true;
            m_is_responded = false;
            nfc_tag_14a_note_radio_activity();
        }
    } else {
...
void nfc_tag_14a_session_recovery(void) {
    if (!m_session_recovery_pending) {
        return;
    }
    if (g_is_tag_emulating) {
        return;
    ...
    if (get_device_mode() != DEVICE_MODE_TAG) {
        m_session_recovery_pending = false;
        return;
    }
    if (nfc_tag_14a_field_present()) {
        return;
    ...
    if (m_diag_recovery < 0xFFFF) m_diag_recovery++;
    nfc_tag_14a_note_radio_activity();
