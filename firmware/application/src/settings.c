...
void settings_init_button_long_press_config(void) {
    config.button_a_long_press = SettingsButtonToggleAutoAdvance;
...
void settings_migrate_button_a_long_press(void) {
    if (config.button_a_long_press == SettingsButtonCloneIcUid) {
        config.button_a_long_press = SettingsButtonToggleAutoAdvance;
        NRF_LOG_INFO("Migrate button A long press: CloneIcUid -> ToggleAutoAdvance");
    }
}

void settings_init_ble_idle_config(void) {
    config.ble_idle_seconds = SETTINGS_BLE_IDLE_DEFAULT_S;
}
...
void settings_migrate(void) {
    switch (config.version) {
        ...
        case 6:
            settings_migrate_button_a_long_press();
...
uint16_t settings_get_ble_idle_seconds(void) {
    return config.ble_idle_seconds;
}

void settings_sanitize_ble_idle(void) {
    const uint16_t v = config.ble_idle_seconds;
    if (settings_ble_idle_seconds_valid(v)) return;
    NRF_LOG_WARNING("ble_idle: stored window %u out of range, using default", (unsigned)v);
    config.ble_idle_seconds = SETTINGS_BLE_IDLE_DEFAULT_S;
}

bool settings_ble_idle_seconds_valid(uint16_t seconds) {
    if (seconds == 0u) return true;
    return seconds >= SETTINGS_BLE_IDLE_MIN_S && seconds <= SETTINGS_BLE_IDLE_MAX_S;
}

void settings_set_ble_idle_seconds(uint16_t seconds) {
    config.ble_idle_seconds = seconds;
}
