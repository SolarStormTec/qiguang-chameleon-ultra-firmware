...
APP_TIMER_DEF(m_sky_reboot_timer);
static void sky_reboot_timer_handle(void *arg);
static void sky_button_reboot_note_edge(void);
...
static void field_generator_rainbow_loop(void) {
    ...
    if (rgb_marquee_slot_switch_busy()) return;
...
static void button_init(void) {
    ...
    err_code = app_timer_create(&m_sky_reboot_timer, APP_TIMER_MODE_SINGLE_SHOT, sky_reboot_timer_handle);
    APP_ERROR_CHECK(err_code);
...
static void system_off_enter(void) {
    ...
    if (g_is_low_battery_shutdown) {
        ...
        uint8_t led = led_index_of_slot(slot);
        uint8_t dir = led > 3 ? 1 : 0;
        ...
        if (animation_config == SettingsAnimationModeFull) {
            if (m_system_off_processing) rgb_marquee_sweep_from_to(color, led, dir ? 7 : 0);
            ...
            if (m_system_off_processing) rgb_marquee_symmetric_in(color, led);
        }
        rgb_marquee_stop();
        if (!m_system_off_processing) {
            for (uint8_t i = 0; i < RGB_LIST_NUM; i++) {
...
static void check_wakeup_src(void) {
    ...
    if (m_reset_source & NRF_POWER_RESETREAS_OFF_MASK) {
        ...
        if (animation_config == SettingsAnimationModeFull) {
            ...
            rgb_marquee_sweep_to(color, dir, 11);
            rgb_marquee_sweep_to(color, !dir, dir ? led_index_of_slot(slot) : (uint8_t)(7 - led_index_of_slot(slot)));
        } else if (animation_config == SettingsAnimationModeMinimal) {
            rgb_marquee_sweep_to(color, !dir, dir ? led_index_of_slot(slot) : (uint8_t)(7 - led_index_of_slot(slot)));
        } else if (animation_config == SettingsAnimationModeSymmetric) {
        ...
        sky_cycle_apply_light_preserving();
...
static void cycle_slot(bool dec) {
    ...
    sky_event_note_slot_changed(slot_new, SKY_SLOT_CHANGE_BY_BUTTON);
}
...
static void run_button_function_by_settings(settings_button_function_t sbf) {
    switch (sbf) {
        case SettingsButtonToggleAutoAdvance:
            sky_cycle_toggle();
            break;
...
#define SKY_REBOOT_HOLD_MS 150u

static volatile bool m_sky_reboot_armed = false;

static void sky_reboot_timer_handle(void *arg) {
    (void)arg;
    if (nrf_gpio_pin_read(BUTTON_2) != 0 && nrf_gpio_pin_read(BUTTON_1) != 0) {
        m_sky_reboot_armed = true;
    }
}
...
static void sky_button_reboot_note_edge(void) {
    ...
    const ret_code_t err = app_timer_start(m_sky_reboot_timer, APP_TIMER_TICKS(SKY_REBOOT_HOLD_MS), NULL);
    if (err != NRF_SUCCESS) {
        NRF_LOG_WARNING("SKY: reboot timer start failed %d", (int)err);
    }
}

static void sky_button_reboot_process(void) {
    if (!m_sky_reboot_armed) {
        return;
    }
    NRF_LOG_WARNING("SKY: A+B held %ums -> restart", (unsigned)SKY_REBOOT_HOLD_MS);
    NRF_LOG_FLUSH();
    nrf_pwr_mgmt_shutdown(NRF_PWR_MGMT_SHUTDOWN_RESET);
    NVIC_SystemReset();
}
...
static void button_press_process(void) {
    ...
    if (m_is_b_btn_release || m_is_a_btn_release) {
        ...
        if (!m_is_field_on) {
            sleep_timer_start(sky_cycle_is_enabled() ? SKY_CYCLE_AWAKE_MS : SLEEP_DELAY_MS_BUTTON_CLICK);
        }
    }
}
...
static void blink_usb_led_status(void) {
    if (rgb_marquee_slot_switch_busy()) return;
    uint8_t slot = tag_emulation_get_slot();
    uint8_t color = sky_cycle_led_color(slot);
    uint8_t dir = slot > 3 ? 1 : 0;
...
int main(void) {
    ...
    while (1) {
        ...
        nfc_tag_14a_session_recovery();

        nfc_tag_14a_field_resume_process();

        nfc_tag_14a_field_reconcile();
        // Log print process
